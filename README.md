...
### LA Gateway cities — keyword research (KWFinder, Los Angeles County)

Batch 2. Two cities cleared research and are built; five did not and are held.

**Cerritos** — built, `published: true`.

| Keyword | Vol | KD |
|---|---|---|
| `windshield repair cerritos` | 10 | 10 |

Lowest KD seen across any city so far. Low volume on its own, but ranking pages
are missing the exact phrase in their titles and there is a Map Pack plus PAA on
the query — worth the page despite the volume.

**Whittier** — built, `published: true`. Title phrase is
"Windshield Repair Whittier, CA", generated by the city template.

| Keyword | Vol | KD | Placement |
|---|---|---|---|
| `windshield repair whittier` | 20 | 12 | Title + H1 |
| `windshield repair whittier ca` | 10 | 15 | Same page, no separate build |
| `auto glass repair whittier` | 30 | 12 | Copy / FAQ |
| `auto glass repair in whittier ca` | 30 | 15 | Copy / FAQ |
| `car window repair whittier` | 30 | 12 | FAQ — see the caveat below |

Map Pack plus PAA on every variant.

**`car window repair` is a scope question, not just a keyword.** The phrase
implies side and rear glass. Nothing on this site claims that service, and the
client has not confirmed it. The Whittier FAQ therefore answers the query
honestly — windshield work is what it describes, and a side or rear window is
"text us a photo and we'll tell you whether we can take it on." If the client
confirms they do side and rear glass, that answer should be rewritten and the
term targeted properly; until then it must not be implied.

**Held — do not build.** Lakewood, Long Beach, Norwalk, Downey and La Mirada.
Both keyword exports for these came back with off-topic autocomplete results
(neighborhood names, assisted-living listings) rather than glass terms. They
stay `published: false` in `src/data/cities.ts` so the slugs are reserved.
Re-pull one city per search, seeded with the full service phrase:

```
windshield repair lakewood ca
windshield repair long beach ca
windshield repair norwalk ca
windshield repair downey ca
windshield repair la mirada ca
```
